...
contract Staking {
    ...
    modifier onlyValidatorOwner(address valAddr) {
        require(validators[validateByAddress[valAddr]].operatorAddress == msg.sender, "sender not validator owner");
        _;
    }
    
    constructor(uint maxVal, uint256 _maxMissed, uint256 _downtimeJailDuration) public{
        maxValidator = maxVal;
    ...
    function getCurrentValidatorSet() view public returns (address[] memory, uint256[] memory) {
    ...
    function finalizeCommit(
        ...
        // allocateTokens
        if (block.number > 1) {
            allocateTokens(previousTotalPower, previousPrecommitTotalPower, addresses, powers);
    ...
    function allocateTokens(
        ...
        uint256 previousFractionVotes = previousPrecommitTotalPower.divTrun(previousTotalPower);
        
        // calculate previous proposer reward
        uint256 proposerMultiplier = baseProposerReward.add(bonusProposerReward.mulTrun(previousFractionVotes));
        ...
        for (uint i = 0; i < addresses.length; i ++) {
            ...
            uint256 reward = feeCollected.mulTrun(voteMultiplier).divTrun(powerFraction);
    ...
    function handleValidateSignature(address valAddr, uint256 power, bool signed) internal {
    ...
    function withdrawDelegationRewards(address delAddr, address valAddr) private returns (uint256) {
        Validator storage val = validators[validateByAddress[valAddr]-1];
        Delegation storage del = delegations[valAddr][delAddr];
        
        val.cumulativeRewardRatio += val.rewards.divTrun(val.tokens);
        del.stake = del.stake.mulTrun(val.slashFractionDowntimeRatio.sub(del.slashFractionDowntimeRatio));
        
        uint256 difference = val.cumulativeRewardRatio.sub(del.cumulativeRewardRatio);
    ...
    function withdrawDelegationReward(address valAddr) public returns (uint256){
    ...
    function updateValidator( uint256 commissionRate, uint256 minselfDelegation) public {
        ...
        if (minselfDelegation > 0) {
            require(minselfDelegation > val.minselfDelegation, "min self delegation recreased");
            require(minselfDelegation < val.tokens, "min self delegation below minumum");
    ...
    function unjail() public {
        ...
        require(del.stake > val.minselfDelegation, "selt delegation too low to unjail");
